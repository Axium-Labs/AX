...
struct ProjectMetadata { id: String }
...
pub(crate) fn load_or_create(root: &Path) -> Result<String> {
    ...
    match fs::read_to_string(&path) {
        Ok(value) => {
            let metadata: ProjectMetadata = serde_json::from_str(&value).context("Invalid project metadata")?;
    ...
    let id = match fs::read_to_string(&legacy) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => uuid::Uuid::new_v4().to_string(),
    ...
    match published {
        Ok(()) => {
            if legacy.exists() { fs::remove_file(legacy)?; }
            Ok(id)
        }
...
mod tests {
    ...
    fn legacy_project_id_migrates_to_json_without_changing_identity() {
        let root = std::env::temp_dir().join(format!("ax-project-migrate-{}", uuid::Uuid::new_v4()));
        ...
        let metadata: ProjectMetadata = serde_json::from_str(&fs::read_to_string(root.join(".ax/project.json")).unwrap()).unwrap();
